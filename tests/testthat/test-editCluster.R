...
test_that("Edit time series", {
  ...
  res <- fread(file.path(opts$inputPath, "thermal", "series", area, cluster_name, "series.txt"))
...
test_that("Edit pre-process data", {
  ...
  res <- fread(file.path(opts$inputPath, "thermal", "prepro", area, cluster_name, "data.txt"))
  m_res <- as.matrix(res)
  dimnames(m_res) <- NULL
...
test_that("Edit pre-process modulation", {
  ...
  res <- fread(file.path(opts$inputPath, "thermal", "prepro", area, cluster_name, "modulation.txt"))
  m_res <- as.matrix(res)
  dimnames(m_res) <- NULL
  expect_equal(m_res, m)
})
