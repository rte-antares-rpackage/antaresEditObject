...
editArea <- function(name, 
  ...
  if (is_api_study(opts)) {
    ...
    if (!is.null(nodalOptimization)) {
      for (property in names(nodalOptimization)) {
        cmd <- api_command_generate(
          ...
          target = sprintf("input/areas/%s/optimization/nodal optimization/%s", name, property),
          data = nodalOptimization[[property]]
        )
    ...
    if (!is.null(nodalThermal)) {
      for (property in names(nodalThermal)) {
        cmd <- api_command_generate(
          action = "update_config", 
          target = sprintf("input/thermal/areas/%s/%s", property, name),
          data = nodalThermal[[property]]
        )
        api_command_register(cmd, opts = opts)
        `if`(
          should_command_be_executed(opts), 
          api_command_execute(cmd, opts = opts, text_alert = "Update area's energy cost option: {msg_api}"),
          cli_command_registered("update_config")
        )
      }
    }
    ...
    if (!is.null(filtering)) {
      for (property in names(filtering)) {
        cmd <- api_command_generate(
          action = "update_config", 
          target = sprintf("input/areas/%s/optimization/filtering/%s", name, property),
          data = filtering[[property]]
        )
        api_command_register(cmd, opts = opts)
        `if`(
          should_command_be_executed(opts), 
    ...
    if (opts$antaresVersion >= 830){
      if (!is.null(adequacy)) {
        for (property in names(adequacy)) {
          cmd <- api_command_generate(
            action = "update_config", 
            target = sprintf("input/areas/%s/adequacy_patch/adequacy-patch/%s", name, property),
            data = adequacy[[property]]
          )
          api_command_register(cmd, opts = opts)
          `if`(
            should_command_be_executed(opts), 
  ...
  if (!is.null(nodalThermal)) {
    ...
    thermal_areas$unserverdenergycost[[name]] <- nodalThermal[["unserverdenergycost"]]
    thermal_areas$spilledenergycost[[name]] <- nodalThermal[["spilledenergycost"]]
